...
7. **审核**：默认跟随原片分辨率和帧率渲染，创作者只需审核最终成片，通过后完成交付。
...
用于人物与公司证据、关键数字、时间线、流程、因果关系、对比、决策、观点引用和无结构观点陈述等常见表达。组件针对口播视频布局，并为人物画面和字幕保留安全区域。“观点陈述”只补充没有更强语义结构的单一判断，全片最多占 25%，且不会连续使用超过两次。
