...
4. **制作**：Agent 连续完成粗剪、字幕、视觉重规划、组件匹配和手绘动画。
5. **自检**：Agent 内部审核粗剪、关键帧、视觉 QA 和技术证据，发现可恢复问题时自动修改并继续。
6. **审核**：默认跟随原片分辨率和帧率渲染，创作者只需审核最终成片，通过后完成交付。
...
- **下游自主视觉规划**：上游视觉方案只作参考；制作 Agent 根据口播语义、已有素材和实际布局重新规划。只有用户自行标注的素材是必须保留的硬约束。
