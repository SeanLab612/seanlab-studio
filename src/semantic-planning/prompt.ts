...
export const createSemanticNarrativePrompt = (
  ...
  system: [
    ...
    "For rhetoric=media-comparison, every item must have a non-empty stable entityId and a non-empty evidence-grounded detail. For a registered project screenshot, entityId must be that exact imageEvidenceInventory assetId so the local renderer can bind its frozen image. For a named public identity, use its stable identity id. Never invent an id; if neither source exists, use another rhetoric or omit the segment.",
    ...
    "Editorial-statement is a restrained coverage bridge, not a default. Its segments may last 2-8 seconds and may appear up to three times consecutively when each statement is semantically suitable. There is no whole-video share cap: keep selecting it for semantically suitable plain-language claims even after the 80 percent minimum has been reached, while always preferring stronger materials and specialized components.",
